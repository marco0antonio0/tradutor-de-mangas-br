...
export async function PATCH(request: Request, { params }: RouteParams) {
  ...
  if (!Number.isFinite(limite)) {
    return NextResponse.json(
      { message: 'Limite inválido', error: 'Bad Request', statusCode: 400 },
      { status: 400 }
    )
  }

  const existing = db.prepare('SELECT id, gerado, limit_page_upload FROM users WHERE id = ?').get(userId) as {
    id?: number
    gerado?: number
  ...
  db.prepare('UPDATE users SET limite = ?, updated_at = ? WHERE id = ?').run(
    limite,
